 ...
 */
class MockError {
  constructor(message, mock, location) {
  ...
  getMessage() {
    ...
    // 这里将异常消息序列化，如果使用直接抛出的异常消息，则会因为使用的toString，对象等会变成[object Object]
  ...
  getStack() {
    const { originalMessage: original, name, message, mock, location = '' } = this
    const { method, path, file } = mock
    const originalStack = original instanceof Error ? original.stack : null
    let stack = ''
    if (typeof originalStack === 'string') {
      const errSource = (originalStack.match('at [^\r\n]+\r?\n') || [''])[0]
      if (errSource.replace(/\\/g, '/').indexOf(file) !== -1) {
        // 异常源是从mock处理函数里抛出的（通过new Error）
        stack = errSource
      }
    }
    if (!stack) {
      // 没有捕获到异常源（没有使用new Error）
      // 使用mock对象来定位异常方法（不能到方法内的具体异常行）
      stack = `at ${method.toUpperCase()} ${path} (${file}${
        (location.match(/^.*?(:\d+:\d+)$/) || ['', ''])[1]
      })\n`
    }
...
module.exports = class MockMiddleware {
  ...
  shouldMock(req) {
    ...
    if (!req.xhr && method === 'GET') {
      ...
      // 公共静态资源
      const appPublicFolders = this.appPublicFolders
  ...
  createHandler(mock) {
    ...
    const middleware = BODY_PARSED_METHODS.includes(method)
      ...
      : []
    //
    middleware.push(this.handleMockData(mock))
    //
    return (req, res, next) => {
      // 对当前请求应用中间件
  ...
  handleMockData(mock) {
    return (req, res, next) => {
      ...
      const doNext = (err) => {
        ...
        // 这里对mock处理程序中调用next时的参数进行筛选
        // express的规则是，第一个参数有值的话，如果不是"route"，则当成错误来处理
        // 这里对这个参数值进行检查
        if (err !== null && err !== undefined && err !== 'route' && err !== 'router') {
          err = new MockError(err, mock, this.maker.getMockLocation(mock, true))
        }
        const fn = next
        // 防止重复被调用
        next = null
      ...
      if (typeof data === 'function') {
        ...
        } else if (mockRes instanceof Error) {
          doNext(mockRes)
